...
export function SettingsPage() {
  ...
  return <section className="settings-page">
    ...
    <Section icon={<Volume2 size={17} />} title="即时释义" description="不捆绑来源不明的词库；可导入你有权使用的 JSON 词典。"><div className="settings-actions"><button className="secondary-button" type="button" onClick={() => void importDictionary()}><Upload size={15} />导入本地词典</button><button className="secondary-button" type="button" onClick={() => void clearDict()}><Eraser size={15} />清除词典缓存</button></div><div className="field settings-remote-url"><label>远程词典 URL 模板（可含 {'{term}'}）</label><input className="text-input" value={settings.dictionaryRemoteUrl} placeholder="https://example.com/dictionary?term={term}" onChange={(event) => patchSettings({ dictionaryRemoteUrl: event.currentTarget.value })} /></div></Section>
